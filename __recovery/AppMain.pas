...
uses
  ...
  Dialogs, Menus, StdCtrls, ExtCtrls, Grids, MsXML, DateUtils, ComCtrls,
  IniFiles,
...
type
  TFormMain = class(TForm)
    ...
    procedure GridSocketInfoDrawCell(Sender: TObject; ACol, ARow: Integer;
  ...
  public
    ...
    procedure CltOnRead(ClientThread: TClientThread; const Buffer: PChar;
      const BufLen: UINT);
    ...
    // 用户离开事件
    procedure UserReadBuffer(UserOBJ: TObject; const Buffer: PChar;
      var BufLen: DWORD; var Succeed: BOOL);
...
uses
  ...
  SHSocket, uTeaSet, Mars, jcldebug;
...
begin
  if g_fCanClose then
    Exit;
  if Application.MessageBox('是否确认退出服务器？', '确认信息', MB_YESNO + MB_ICONQUESTION) = IDYES
  then
  begin
...
begin
  ...
  begin
    ...
    GridSocketInfo.DefaultDrawing := False;
...
procedure TFormMain.GridSocketInfoDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
begin
    ...
    begin
      ...
      begin
        ...
        if (UserOBJ <> nil) and (UserOBJ.m_tLastGameSvr <> nil) and
          ...
          ListBoxActiveList.Items.AddObject(Trim(UserOBJ.m_pUserOBJ.pszIPAddr),
            TObject(UserOBJ));
      end;
    end;
    ListBoxTempList.Clear;
    for I := 0 to g_TempBlockIPList.Count - 1 do
      ListBoxTempList.Items.AddObject(g_TempBlockIPList.Strings[I],
        g_TempBlockIPList.Objects[I]);

    ListBoxBlockList.Clear;
    for I := 0 to g_BlockIPList.Count - 1 do
      ListBoxBlockList.Items.AddObject(g_BlockIPList.Strings[I],
        g_BlockIPList.Objects[I]);

    ListBoxIPAreaFilter.Clear;
    for I := 0 to g_BlockIPAreaList.Count - 1 do
      ListBoxIPAreaFilter.Items.AddObject(g_BlockIPAreaList.Strings[I],
        g_BlockIPAreaList.Objects[I]);

    etMaxConnectOfIP.Value := m_nMaxConnectOfIP;
    etClientTimeOutTime.Value := m_nClientTimeOutTime div 1000;
    case m_tBlockIPMethod of
      mDisconnect:
        rdDisConnect.Checked := True;
      mBlock:
        rdAddTempList.Checked := True;
      mBlockList:
        rdAddBlockList.Checked := True;
    end;
...
begin
  ...
  begin
    ...
    begin
      ClientThread := m_xGameServerList.InitGameServer
        (g_pConfig.m_xGameGateList[I].nServerPort,
        g_pConfig.m_xGameGateList[I].sServerAdress);
      ClientThread.m_nPos := I;
      ClientThread.OnReadEvent := CltOnRead;
      ClientThread.OnCloseEvent := CltOnClose;
      m_xRunServerList.InitServer(g_pConfig.m_xGameGateList[I].nGatePort,
        ClientThread);
...
procedure TFormMain.CltOnRead(ClientThread: TClientThread; const Buffer: PChar;
  const BufLen: UINT);
var
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              UserOBJ.ProcessSvrData(ClientThread, Integer(pTRBuf + nPos + 1),
                nBufLen - nPos - 1);
            end;
          end;
        end;
...
begin
  if g_pLogMgr.CheckLevel(5) then
    g_pLogMgr.Add('服务器连接已关闭: ' + TClientThread(Sender).ServerIP + ':' +
      IntToStr(TClientThread(Sender).ServerPort));
  with m_xRunServerList.IOCPServer do
...
procedure TFormMain.UserEnterEvent(Sender: TUserManager;
  const UserOBJ: pTUserOBJ);
var
...
procedure TFormMain.UserLeaveEvent(Sender: TUserManager;
  const UserOBJ: pTUserOBJ);
begin
...
procedure TFormMain.UserReadBuffer(UserOBJ: TObject; const Buffer: PChar;
  var BufLen: DWORD; var Succeed: BOOL);
label
  LOOP;
var
...
begin
  ...
  // Log time stamp
  // writeWorkLog('1111111111111111111');
  MemoLog.Lines.Add(DateTimeToStr(Now));

  // Log unhandled exception stack info to ExceptionLogMemo
  JclLastExceptStackListToStrings(MemoLog.Lines, False, True, True, False);
