...
type
  ...
  TFreeMemEvent = procedure(Sender: TObject; const MemBuffer: PWideChar)
    of object;
  ...
  protected
    ...
    procedure BuilderMemoryList(BaseMemoryNode: PXMemNode;
      BaseMemory: PXMemItem; MemoryIndex: Integer;
      BaseBlockCount: Integer = DEFAULT_MEMORY_COUNT);
  ...
  public
    ...
    property OnDestroyEvent: TFreeMemEvent read FOnDestroyEvent
      write FOnDestroyEvent;
    procedure LockMemPool;
    procedure UnLockMemPool;
    procedure FirstMemory;
    procedure NextMemory;
    function MemoryItem: PWideChar;
    ...
    constructor Create(MemoryBlockSize: Integer;
      MemoryBlockCount: Integer = DEFAULT_MEMORY_COUNT);
...
procedure TFixedMemoryPool.BuilderMemoryList(BaseMemoryNode: PXMemNode;
  // 头的内存节点表
  BaseMemory: PXMemItem; MemoryIndex: Integer;
...
begin
  ...
  begin
    ...
    if i = 0 then
      BuilderMemoryList(PTRClearNode, PTRClearNode.MemItem, 1,
        FReAllocMemCount + 1)
    else
      BuilderMemoryList(PTRClearNode, PTRClearNode.MemItem,
        FReMemPosArray[i - 1].MaxIndex + 1, FReAllocMemCount);
    ...
    if i < FReAllocCount - 1 then
      // 设置尾节点的 后一节点
      PXMemNode(FReMemPosArray[i].HPos).NextNode :=
        PXMemNode(FReMemPosArray[i + 1].LPos);
...
constructor TFixedMemoryPool.Create(MemoryBlockSize: Integer;
  MemoryBlockCount: Integer = DEFAULT_MEMORY_COUNT);
var
...
begin
  ...
  FMemRealHeader := HeapAlloc(GetProcessHeap(), $8,
    FRealBlockCount * SizeOf(TXMemNode));
...
begin
  ...
  begin
    ...
    begin
      Result := Pointer(FReMemPosArray[i].LPos +
        (Integer(ID) - FReMemPosArray[i].MinIndex) * SizeOf(TXMemNode));
      break;
    end;
  end;
end;
...
begin
  ...
  for i := 0 to FReAllocCount - 1 do
  begin
    if (KeyID <= FReMemPosArray[i].HMPos) and (KeyID >= FReMemPosArray[i].LMPos)
    then
    begin
...
function TFixedMemoryPool.MemoryItem: PWideChar;
begin
...
begin
  ...
  // $8 is HEAP_ZERO_MEMORY	but delphi no define this value
  FReHeader := HeapAlloc(GetProcessHeap(), $8,
    FReAllocMemCount * SizeOf(TXMemNode));

  if FReHeader = nil then
    Exit;

  FReMemory := HeapAlloc(GetProcessHeap(), $8, FReAllocMemCount * FBlockSize);

  if FReMemory = nil then
    Exit;

  BuilderMemoryList(FReHeader, FReMemory, FReMemPosArray[FReAllocCount].MaxIndex
    + 1, FReAllocMemCount);
