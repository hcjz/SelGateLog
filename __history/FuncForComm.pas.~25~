...
procedure OnTimerProc(hWnd: hWnd; uMsg: UINT; idEvent: UINT_PTR;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          if (GetTickCount - CSession.m_dwClientTimeOutTick >
            g_pConfig.m_nClientTimeOutTime) then
          begin
            CSession.m_dwClientTimeOutTick := GetTickCount;
            CSession.SendDefMessage(SM_OUTOFCONNECTION, CSession.m_nSvrObject,
              0, 0, 0, '');
            CSession.m_fKickFlag := True;
            BlockUser(CSession);
            if g_pLogMgr.CheckLevel(5) then
            begin
              g_pLogMgr.Add('Client Connect Time Out: ' +
                CSession.m_pUserOBJ.pszIPAddr);
            end;
          end;
        end;
      end
...
//         ml:= mylog.create;
//        ml.log(pchar('12222'));
//    //writeWorkLog(e.StackTrace);
//    JclLastExceptStackListToStrings(FormMain.MemoLog.Lines, False, True, True, False);
...
begin
  ...
  try
    ...
    begin
      ...
      if FormMain.m_xGameServerList[i].Active and FormMain.m_xGameServerList[i].m_fKeepAlive
      then
      begin
        if GetTickCount() - FormMain.m_xGameServerList[i].m_dwKeepAliveTick > 25
          * 1000 then
        begin
  ...
  begin
    ...
    begin
      ...
      case PSI.pClient.m_tSockThreadStutas of
        stConnecting:
          FormMain.GridSocketInfo.Cells[2, nRow] := '连接中..';
        stConnected:
          FormMain.GridSocketInfo.Cells[2, nRow] := '已连接';
        stTimeOut:
          FormMain.GridSocketInfo.Cells[2, nRow] := '超时';
      end;

      FormMain.StatusBar.Panels[0].Text :=
        Format('连接: %d/%d', [IOCPAccepter.InUseBlock,
